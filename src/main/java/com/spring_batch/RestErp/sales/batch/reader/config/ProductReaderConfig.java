package com.spring_batch.RestErp.sales.batch.reader.config;

import com.spring_batch.RestErp.sales.batch.reader.itemReader.ProductItemReader;
import com.spring_batch.RestErp.sales.dto.source.ProductSource;
import org.springframework.batch.item.ItemReader;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

import javax.sql.DataSource;

@Configuration
public class ProductReaderConfig {

    @Bean
    public ItemReader<ProductSource> productReader(
            @Qualifier("erpDataSource") DataSource erpDataSource) {
        return new ProductItemReader(erpDataSource);
    }
}